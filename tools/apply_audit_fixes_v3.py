#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Temporary whitespace-safe patcher for TravelRef FR audit fixes."""
...
def one(path: str, old: str, new: str, expected: int = 1) -> None:
    ...
    if n == 0 and new in text:
        return
    if n != expected:
        raise RuntimeError(f"{path}: expected {expected}, got {n}: {old!r}")
    p.write_text(text.replace(old, new), encoding="utf-8")


# TR_Main: load FR before UI, keep area keys internal, and route display text
# through the localization helpers.
P = "Dusk/TravelRef/TR_Main.lua"
one(P, 'import "Dusk.TravelRef.TR_Data"\n\nfunction print(text)',
    ...
    'function print(text)')
one(P, 'print(str.."(ST): "..d.n..l)', 'print(str.."(ST): "..TR_LocName(d.n)..l)')
one(P, '\tif Dusk.TravelRef.Common.HelpCmd(cmd,args,help) then return end\n\targs = TR_LocKey(args)',
    '\tif Dusk.TravelRef.Common.HelpCmd(cmd,args,help) then return end\n\tlocal rawArgs = args\n\targs = TR_LocKey(args)')
one(P, '\tif cmd=="tra" then\n\t\tif args~="" then\n\t\t\tlocal str=args:lower()',
    '\tif cmd=="tra" then\n\t\tif rawArgs~="" then\n\t\t\tlocal str=rawArgs:lower()')
one(P, '\t\t\tfor area,zone in pairs(Areas) do\n\t\t\t\tif area:lower():find(str,1,true) then\n\t\t\t\t\tprint(area.." dans "..TR_ZoneName(zone))',
...
one(P, 'if not ln then print("(none found)")', 'if not ln then print("(aucun trouvé)")')
one(P, '\t\tfor area,zone in Sort(Areas) do\n\t\t\tprint(area.." dans "..TR_ZoneName(zone))',
    '\t\tfor area,zone in Sort(Areas) do\n\t\t\tprint(TR_AreaName(area).." dans "..TR_ZoneName(zone))')
one(P, 'print(string.format(xlink,id,tbl.nm.." "..n)..s)', 'print(string.format(xlink,id,tbl.nm.." "..TR_LocName(n))..s)', expected=2)
one(P, 'elseif t.r then print(tbl.nm.." "..n.." (Trait)")', 'elseif t.r then print(tbl.nm.." "..TR_LocName(n).." (Trait)")')
one(P, 'if t.a then sz = " in "..t.a end', 'if t.a then sz = " dans "..TR_AreaName(t.a) end')
one(P, 'if t.a then z = string.format("%s(%s)", t.a,z) end', 'if t.a then z = string.format("%s (%s)", TR_AreaName(t.a),z) end')
one(P,
...
one(P, 'd = string.format(" (%.1f units away).",d1)', 'd = string.format(" (à %.1f unités).",d1)')

# TR_Window route summaries.
P = "Dusk/TravelRef/TR_Window.lua"
one(P, "V.s = skill.nm..' '..rtn..\" -> \"..v.s", "V.s = skill.nm..' '..TR_LocName(rtn)..\" -> \"..v.s")
one(P, 'printh("Recherche du meilleur départ vers "..SetLoc.." :")', 'printh("Recherche du meilleur départ vers "..TR_LocName(SetLoc).." :")')
one(P, 'V.s = "Jalon vers "..name.." -> "..v.s', 'V.s = "Jalon vers "..TR_LocName(name).." -> "..v.s')
one(P, 'local hs = "Voyage vers maison "..hn.." -> Écurie de maison(MT) -> "', 'local hs = "Voyage vers maison "..TR_LocName(hn).." -> Écurie de maison(MT) -> "')
one(P, 'local hs = "Voyage vers maison "..hn.." -> Bateau de maison(MT) -> "', 'local hs = "Voyage vers maison "..TR_LocName(hn).." -> Bateau de maison(MT) -> "')
one(P, 'V.s = hs..name..((v.s and "(ST) -> "..v.s) or "(ST)")', 'V.s = hs..TR_LocName(name)..((v.s and "(ST) -> "..v.s) or "(ST)")', expected=2)

# Launcher no longer imports localization late.
...
# Official FR runtime layer: clean morphology markers, expose special labels,
# and never mutate Locs[].a / Areas.
P = "Dusk/TravelRef/TR_OfficialFR.lua"
one(P, '    local function OfficialNorm(value)\n',
...
# Generator cleanup + two verified legacy-only labels.
P = "tools/generate_official_fr.py"
one(P, 'EAGLE_SUFFIX_RE = re.compile(r"\\s*-\\s*Eagle\\s*$", re.IGNORECASE)\n',
...
one(P, '\ndef fetch_labels(lang: str, filename: str) -> dict[str, str]:\n',
    ...
    'def fetch_labels(lang: str, filename: str) -> dict[str, str]:\n')
one(P, '        node.attrib["key"]: html.unescape(node.attrib.get("value", "")).strip()\n',
...
one(P,
    ...
    '            else:\n'
    '                unresolved.append(name)')
one(P, '        "-- English and French values are joined by identical localization IDs.",',
...
one("Dusk/TravelRef.plugincompendium", '<Version>3.3.2</Version>', '<Version>3.3.2-FR-r4</Version>')

# Release notes.
p, text = load("Dusk/TravelRef/TR_FR_NOTES.txt")
if "Correctifs FR r4\n----------------\n" not in text:
...
print("TravelRef audit fixes v3 applied successfully.")
